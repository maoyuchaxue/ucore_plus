...
void hypercall_io_clear(hypercall_io *hio){ // checked
	memset(hio, 0, sizeof(hypercall_io)) ;
}

void hypercall_io_init(hypercall_io *hio){ // checked
...
void hypercall_debug_quiet(int current_cpu, char *format, ...) // checked
{
	hypercall_io hio;
	int len;
	...
	if(STD_SKI_HYPERCALLS){
		...
		cprintf("[%d] Would have issued msg: %s (hypercall disabled)\n", current_cpu, hio.p.hio_debug.gh_msg);
	}
	va_end(list);
}
...
/*
print dmesg info, which doesn't appear in Ucore
...
void hypercall_export_time(int current_cpu){ // checked
	int sec;
	char str[128];

	sec = gettime_msec(NULL)/1000;

	snprintf(str, 100, "Seconds", sec);
	hypercall_debug(current_cpu, str);

	//snprintf(str, 100, ctime(&sec));
	//hypercall_debug(current_cpu, str);
}
