...
NORETURN PRINTF static void fail(const char* msg, ...) // checked
{
	int e = errno;
	va_list args;
	va_start(args, msg);
...
NORETURN PRINTF static void error(const char* msg, ...) // checked
{
	va_list args;
	va_start(args, msg);
	vfprintf(stderr, msg, args);
...
NORETURN PRINTF static void exitf(const char* msg, ...) // checked
{
	int e = errno;
	va_list args;
	va_start(args, msg);
...
static void csum_inet_init(struct csum_inet* csum) //checked
{
	csum->acc = 0;
}

static void csum_inet_update(struct csum_inet* csum, const uint8* data, size_t length) // checked
{
	if (length == 0)
		return;
...
static uint16 csum_inet_digest(struct csum_inet* csum) // checked
{
	return ~csum->acc;
}
#endif
