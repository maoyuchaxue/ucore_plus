...
enum sandbox_type flag_sandbox;
...
struct thread_t threads[kMaxThreads];
...
struct res_t results[kMaxCommands];
...
long execute_syscall(struct call_t* c, long a0, long a1, long a2, long a3, long a4, long a5, long a6, long a7, long a8);
struct thread_t* schedule_call(int call_index, int call_num, bool colliding, uint64 copyout_index, uint64 num_args, uint64* args, uint64* pos);
void handle_completion(struct thread_t* th);
void execute_call(struct thread_t* th);
void thread_create(struct thread_t* th, int id);
void* worker_thread(void* arg);
uint32* write_output(uint32 v);
void write_completed(uint32 completed);
uint64 read_input(uint64** input_posp);
uint64 read_input2(uint64** input_posp, bool peek);
...
void cover_open();
void cover_enable(struct thread_t* th);
void cover_reset(struct thread_t* th);
uint32 read_cover_size(struct thread_t* th);
static uint32 hash(uint32 a);
static bool dedup(uint32 sig);

void setup_control_pipes() // checked
{
	// 把文件描述符的映射进行更改：stdin转移至kInPipeFd, stdout转移至kOutPipeFd，而0,1则统一变为stderr
...
// 接受一次execute指令
void receive_execute(bool need_prog) // checked
{
	struct execute_req *req_p = (struct execute_req *)(&(inpipe_data[0]));
	while (1) {
		kAFL_hypercall(HYPERCALL_KAFL_GET_INPIPE, (uint64_t)inpipe_data);
	...
	// we do not support read from pipe, because it's not actually a pipe at all
...
void execute_one()
{
	...
	kAFL_hypercall(HYPERCALL_KAFL_GET_PAYLOAD, (uint64_t)input_data);
...
retry:
	...
	for (;;) { 
		...
		for (uint64 i = num_args; i < 6; i++)
			args[i] = 0;
		struct thread_t* th = schedule_call(call_index++, call_num, colliding, copyout_index, num_args, args, input_pos);

		if (colliding && (call_index % 2) == 0) {
			// Don't wait for every other call.
			// We already have results from the previous execution.
		} else if (flag_threaded) { // 先不考虑多线程
		/*
...
// 把call放进线程池中的某个线程之中
// 线程池中一共有kMaxThreads个线程，但是由于执行的是单线程模式，最终应该只会顺序调用一个
// 事实上ucore有多线程的功能，所以在之后可以考虑加入多线程模式（虽然还缺了一个系统调用）
struct thread_t* schedule_call(int call_index, int call_num, bool colliding, uint64 copyout_index, uint64 num_args, uint64* args, uint64* pos)
{
...
// 暂且认定系统就是64位的
// 写覆盖信息
#define cover_t uint64
void write_coverage_signal(struct thread_t* th, uint32* signal_count_pos, uint32* cover_count_pos) 
{
	// Write out feedback signals.
	// Currently it is code edges computed as xor of two subsequent basic block PCs.
	debug("Write_coverage_signal of cover_size: %llu\n", th->cover_size) ;
	...
	*signal_count_pos = nsig;
	...
	if (flag_dedup_cover) { 
		...
		// use bubble sort and unique
		for(cover_t* i = cover_data; i != end; i ++)
			for(cover_t* j = i; j != end; j ++)
				if(*i > *j) 
				{
					cover_t t ;
					t = *i, *i = *j, *j = t ;
				}
		cover_t *t = cover_data ;
		for(cover_t* i = cover_data; i != end; i ++)
			if(*i != *t)
				*(++t) = *i ;
		cover_size = (++t) - cover_data;
	}
...
void handle_completion(struct thread_t* th) // checked
{
	...
	if (!collide && !th->colliding) {
		...
		if (flag_collect_comps) {
			...
			*comps_count_pos = comps_size;*/
			...
			write_coverage_signal(th, signal_count_pos, cover_count_pos);
		}
...
void execute_call(struct thread_t* th)
{
	...
	if (th->res == -1 && th->reserrno == 0)
		th->reserrno = 22; // EINVAL our syz syscalls may misbehave
...
static uint32 hash(uint32 a) // checked
{
	a = (a ^ 61) ^ (a >> 16);
	a = a + (a << 3);
	a = a ^ (a >> 4);
...
static bool dedup(uint32 sig) // checked
{
...
bool copyout(char* addr, uint64 size, uint64* res) // checked 
{
	atomic_t ok ; 
	atomic_set(&ok, 0) ;
	NONFAILING(
...
uint64 read_arg(uint64** input_posp) // checked
{
...
uint64 read_const_arg(uint64** input_posp, uint64* size_p, uint64* bf_off_p, uint64* bf_len_p) // checked
{
...
uint64 read_result(uint64** input_posp) // checked
{
...
uint64 read_input2(uint64** input_posp, bool peek) // checked 
{
...
uint64 read_input(uint64** input_posp) // checked 
{
	uint64* input_pos = *input_posp;
	if ((char*)input_pos >= input_data + kMaxInput)
		fail("input command overflows input");
